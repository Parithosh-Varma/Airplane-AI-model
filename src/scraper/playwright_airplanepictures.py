...
class PlaywrightAirplanePicturesScraper(PlaywrightBaseScraper):
    @property
    ...
    async def extract_listings(self, page: Page) -> list[dict]:
        ...
        for link in links:
            ...
            if not href or href in seen:
                continue
            seen.add(href)
            full_url = "https://airplane-pictures.net" + href if href.startswith("/") else href
    ...
    async def extract_detail(self, page: Page, listing: dict) -> dict:
        ...
        try:
            ...
            logger.debug("[airplane-pictures] Detail parse: %s", e)
        return result
