...
class PlaywrightPlanespottersScraper(PlaywrightBaseScraper):
    @property
    ...
    async def extract_listings(self, page: Page) -> list[dict]:
        ...
        for link in links:
            ...
            if not href or href in seen:
                continue
            seen.add(href)
            full_url = "https://www.planespotters.net" + href if href.startswith("/") else href
    ...
    async def extract_detail(self, page: Page, listing: dict) -> dict:
        ...
        try:
            ...
            logger.debug("[planespotters] Detail parse: %s", e)
        return result
